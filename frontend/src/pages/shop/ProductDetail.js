...
import { Heart, ShoppingBag, Star, Minus, Plus, Truck, RefreshCw, Shield, ArrowLeft } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useCart } from '../../context/CartContext';
import { productsAPI, ordersAPI, userAPI, reviewsAPI } from '../../services/api';
...
const ProductDetail = () => {
  ...
  const loadProduct = async () => {
    try {
      ...
      if (productData._id) {
        try {
          const relatedResponse = await productsAPI.getRelatedProducts(productData._id);
              ...
              <div className="variant-selection">
                <div className="color-selection">
                  ...
                  <div className="color-options">
                    {[...new Map(product.variants.map(v => [v.color.name, v.color])).values()].map((color) => (
                      <button
                        key={color.name}
                        ...
                        onClick={() => {
                          ...
                          const availableSizes = product.variants
                            .filter(v => v.color.name === color.name && v.stock > 0)
                            .map(v => v.size);
                ...
                <div className="size-selection">
                  <h3>Size: <span className="selected-value">{selectedSize}</span></h3>
                  <div className="size-options">
                    {[...new Set(product.variants.map(v => v.size))].map((size) => {
                      const isAvailable = selectedColor ? 
                        product.variants.some(v => v.size === size && v.color.name === selectedColor && v.stock > 0) :
            ...
            <div className="purchase-section">
              <div className="quantity-selector">
                ...
                <button
                  ...
                  disabled={!getSelectedVariant() || quantity >= getSelectedVariant()?.stock}
                >
                  <Plus size={16} />
                </button>
              </div>
              ...
              <Button
                ...
                disabled={!selectedSize || !selectedColor || !getSelectedVariant()?.stock}
              >
                <ShoppingBag size={18} />
